...
func (dbo *DatabaseObject) prepareStatements() (err error) {
	...
	// 17.
	q = fmt.Sprintf(`INSERT INTO %s (ThreadId, TEXT, CreatorUserId, CreatorTime) VALUES (?, ?, ?, Now());`, dbo.tableNames.Messages)
	qs = append(qs, q)
	...
	// 19.
	q = fmt.Sprintf(`UPDATE %s SET TEXT = ?, EditorUserId = ?, EditorTime = Now() WHERE Id = ?;`, dbo.tableNames.Messages)
	qs = append(qs, q)
	...
	// 23.
	q = fmt.Sprintf(`SELECT Id, ThreadId, Text, CreatorUserId, CreatorTime, EditorUserId, EditorTime FROM %s WHERE Id = ?;`, dbo.tableNames.Messages)
	qs = append(qs, q)
